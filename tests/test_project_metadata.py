...
def test_v060_release_notes_are_bilingual_and_truthful_about_external_evidence():
    ...
    assert notes.startswith("# Clinical Data Research Navigator v0.6.0\n")
    for heading in (
        "## English",
...
def test_v080_release_notes_are_bilingual_and_describe_the_auditable_ledger():
    notes = (ROOT / "docs/releases/0.8.0.md").read_text(encoding="utf-8")
    normalized = " ".join(notes.split())

    assert notes.startswith("# Clinical Data Research Navigator v0.8.0\n")
    for heading in (
        "## English",
    ...
    for contract in (
        ...
        "exit code 3",
        "不含固定表頭的精簡自然語句",
        "封閉式 JSON evidence ledger",
        "離線檢查器",
        "未知的穩定識別碼使用 null",
        "不重跑 72-cell benchmark",
    ):
        assert contract in normalized
    for unsupported_claim in (
        "proves source truth",
...
def test_benchmark_issue_form_points_to_the_published_v070_tag():
    ...
    repository_version = next(
        item
        for item in payload["body"]
        if item.get("id") == "repository_version"
    )
