...
Codex 會從 `$HOME/.agents/skills` 載入個人 Skill。請從同一個 Release 下載 ZIP 與 manifest，先用 manifest 核對 ZIP，再解壓縮到獨立的 Skill 目錄。以下範例會安裝 `v0.2.1`；遇到既有安裝時會停止，不會直接覆寫。
...
python scripts/install_local.py \
  /absolute/path/you/select/skill-package/clinical-data-research-navigator-0.2.1.zip \
